...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def reset_parameters(self):
        nn.init.kaiming_uniform_(self.lora_encoder_k.weight, a=math.sqrt(5))
        nn.init.zeros_(self.lora_decoder_k.weight)
        nn.init.kaiming_uniform_(self.lora_encoder_q.weight, a=math.sqrt(5))
        nn.init.zeros_(self.lora_decoder_q.weight)
        nn.init.kaiming_uniform_(self.lora_encoder_v.weight, a=math.sqrt(5))
        nn.init.zeros_(self.lora_decoder_v.weight)
        nn.init.kaiming_uniform_(self.lora_encoder_up.weight, a=math.sqrt(5))
        nn.init.zeros_(self.lora_decoder_up.weight)
        nn.init.kaiming_uniform_(self.lora_encoder_down.weight, a=math.sqrt(5))
        nn.init.zeros_(self.lora_decoder_down.weight)
        ...
        for key in key_list:
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    if 'q' in target_name:
                        new_module = Linear(target.in_features, target.out_features, self.lora_encoder_q, self.lora_decoder_q, bias=bias, **kwargs)
                    elif 'k' in target_name:
                        new_module = Linear(target.in_features, target.out_features, self.lora_encoder_k, self.lora_decoder_k, bias=bias, **kwargs)
                    elif 'v' in target_name:
                        new_module = Linear(target.in_features, target.out_features, self.lora_encoder_v, self.lora_decoder_v, bias=bias, **kwargs)
                    elif 'up' in target_name:
                        new_module = Linear(target.in_features, target.out_features, self.lora_encoder_up, self.lora_decoder_up, bias=bias, **kwargs)
                    elif 'down' in target_name:
                        new_module = Linear(target.in_features, target.out_features, self.lora_encoder_down, self.lora_decoder_down, bias=bias, **kwargs)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A") or hasattr(self, "lora_m"):
    ...
    def train(self, mode: bool = True):
        ...
        if self.lora_type == "lora":
            ...
            self.lora_m.train(mode)
            self.lora_encoder.train(mode)
            self.lora_decoder.train(mode)
    def eval(self):
        nn.Linear.eval(self)
        if self.lora_type == 'lora':
            self.lora_A.eval()
            ...
            self.lora_encoder.eval()
            self.lora_decoder.eval()
            self.lora_m.eval()

    def forward(self, x: torch.Tensor):
        #print("forward function:", self.merged)
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                if self.lora_type == 'lora':
                    ...
                    result += self.lora_dropout(self.nolinear(self.lora_decoder(self.lora_m(self.lora_dropout(self.nolinear(self.lora_encoder(self.lora_dropout(x.to(self.lora_m.weight.dtype))))))))) * self.scaling
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
